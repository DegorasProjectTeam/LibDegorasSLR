...
#include <LibDPSLR/FormatsILRS/common/consolidated_types.h>
#include <LibDPSLR/FormatsILRS/common/consolidated_record.h>
#include <LibDPSLR/Timing/time_utils.h>
#include <LibDPSLR/SpaceObject/spaceobject_utils.h>
#include <LibDPSLR/Helpers/Helpers>
// =====================================================================================================================
...
void CPF::getAvailableTimeWindow(long long &mjd_start, long double &secs_start, long long &mjd_end, long double &secs_end) const
{
    if (this->empty_)
    {
        mjd_start = 0LL;
...
math::Interval<long double> CPF::getAvailableTimeInterval() const
{
    ...
    {
        // Get the start time.
        long double mjdt_start = timing::mjdAndSecsToMjdt(this->getData().positionRecords().front().mjd,
                                                          this->getData().positionRecords().front().sod);
        // Get the stop time.
        long double mjdt_stop = timing::mjdAndSecsToMjdt(this->getData().positionRecords().back().mjd,
