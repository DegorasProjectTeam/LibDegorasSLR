...
public:
     ...
     */
    PredictionSLR::ErrorType predict(const dpbase::timing::dates::MJDateTime& mjdt, PredictionSLR& result) const override;
     ...
     */
    void getAvailableTimeWindow(dpbase::timing::dates::MJDateTime &start, dpbase::timing::dates::MJDateTime &end) const override;
     ...
     */
    std::string getErrorMsg(PredictionSLR::ErrorType error_code) const override;


private:
    
    void callInterp(const dpbase::math::units::Seconds& x, dpbase::math::types::Vector3D<dpbase::math::units::Meters>& y,
                    PredictionSLR& result) const;

    static PredictionError convertLagInterpError(dpbase::stats::types::LagrangeError error);

    // Private containers.
    dpbase::math::types::Matrix<dpbase::math::units::Meters> rotm_topo_local_;  // Topocentric local rotation matrix.
    std::vector<dpbase::math::units::Seconds> pos_times_;                       // Position times used at interpolation.
    dpbase::math::types::Matrix<dpbase::math::units::Meters> pos_data_;         // Positions used at interpolation as matrix.
