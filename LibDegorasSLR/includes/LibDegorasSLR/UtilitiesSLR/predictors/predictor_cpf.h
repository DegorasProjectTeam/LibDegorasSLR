...
public:
     ...
     * @param mjd, the modified julian datetime of the instant to be interpolated.
    ...
    SLRPredictions predict(MJDateTime mjdt_start, MJDateTime mjdt_end, unsigned step_ms) const;

    /**
     * @brief If predictor is ready, returns the time window in which the predictor can be used.
     * @param start, Modified Julian Date in days of time window start.
     * @param end, Second of day in seconds of time window start.
     * @param mjd_end, Modified Julian Date in days of time window end.
     * @param sod_end, Second of day in seconds of time window end.
     */
    void getTimeWindow(MJDateTime &start, MJDateTime &end) const;
